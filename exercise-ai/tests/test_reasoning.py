...
def test_openai_compatible_kwargs_grok_always(monkeypatch):
    monkeypatch.delenv('LLM_REASONING_EFFORT', raising=False)
    kw = reasoning.openai_compatible_effort_kwargs(api_tag='grok', model='grok-4.6')
    assert kw == {'reasoning_effort': 'medium'}

def test_openai_compatible_kwargs_openai_omit_gpt4o(monkeypatch, capsys):
    monkeypatch.setenv('LLM_REASONING_EFFORT', 'low')
    kw = reasoning.openai_compatible_effort_kwargs(api_tag='openai', model='gpt-4o-mini')
    assert kw == {}
...
def test_openai_compatible_kwargs_openai_allowlisted(monkeypatch):
    ...
    kw = reasoning.openai_compatible_effort_kwargs(api_tag='openai', model='gpt-5-mini')
    assert kw == {'reasoning_effort': 'medium'}
...
def test_gemini_config_includes_thinking_level(monkeypatch):
    ...
    monkeypatch.setenv('LLM_REASONING_EFFORT', 'low')
    monkeypatch.setattr(generator_gemini, 'GEMINI_MODEL_FALLBACKS', ('gemini-3.1-flash-lite',))
    batch = ExerciseBatch(exercicios=[Exercise(enunciado='1+1', resposta='2', explicacao='soma', dificuldade=DificuldadeEnum.FACIL)], dificuldades=[DificuldadeEnum.FACIL])
    response = MagicMock()
...
def test_cli_reasoning_sets_env(tmp_path, monkeypatch):
    ...
    main_mod.main(['--reasoning', 'high', '--out', str(out), '--provider', 'openai'])
