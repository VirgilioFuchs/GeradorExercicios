...
def test_grok_mapper_auth_stderr_never_contains_key(monkeypatch):
    ...
    buf = io.StringIO()
    with contextlib.redirect_stderr(buf):
        err = generator.map_openai_compatible_error(_AuthLeak(), api_tag='grok', of_label='do Grok', auth_key_name='GROK_API_KEY')
    stderr = buf.getvalue()
...
def test_run_failure_logs_omit_key_values(monkeypatch, tmp_path):
    ...
    from models import DificuldadeEnum, GenerationRequest
    import reliability
    request = GenerationRequest(materia='Matemática', topico='Equação do primeiro grau', dificuldade=DificuldadeEnum.FACIL, quantidade=3)
    out, err = (io.StringIO(), io.StringIO())
    with patch.object(reliability, 'generate_exercises', side_effect=ValueError('quantidade incorreta: esperado 3 exercícios, recebido 1')):
...
def test_duration_aggregate_never_contains_keys(monkeypatch, tmp_path):
    ...
    batch = ExerciseBatch(exercicios=[Exercise(enunciado='e1', resposta='r1', explicacao='x1', dificuldade=DificuldadeEnum.FACIL)], dificuldades=[DificuldadeEnum.FACIL])
    request = GenerationRequest(materia='Matemática', topico='t', dificuldade=DificuldadeEnum.FACIL, quantidade=1)
    err = io.StringIO()
