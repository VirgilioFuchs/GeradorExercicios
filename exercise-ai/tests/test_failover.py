...
# --- Mapper api_error_kind (Task 1 tracer) ---


def test_openai_timeout_eligible_auth_not():
    timeout_err = generator.map_openai_compatible_error(
    ...
    auth_err = generator.map_openai_compatible_error(
        AuthenticationError(
            message="bad key",
...
def test_openai_to_gemini_failover_success(demo_batch, request_demo, monkeypatch):
    ...
    with contextlib.redirect_stderr(err):
        out = failover.generate_with_failover(
            request_demo,
            1,
            generate_fn=fake_gen,
            resolve_provider=lambda: "openai",
            switch_provider=switched.append,
        )
...
def test_secondary_failure_propagates_no_third(request_demo, monkeypatch):
    ...
    err = io.StringIO()
    with pytest.raises(RuntimeError, match="secundário falhou"), contextlib.redirect_stderr(
        err
    ):
        failover.generate_with_failover(
            request_demo,
...
def test_explicit_provider_openai_still_failovers(demo_batch, request_demo, monkeypatch):
    ...
    with contextlib.redirect_stderr(io.StringIO()):
        failover.generate_with_failover(
            ...
            1,
            generate_fn=fake_gen,
            resolve_provider=generator._resolve_provider,
            switch_provider=switched.append,
        )
...
# --- Task 3: main.run --out + redaction ---


def test_run_failover_writes_out_and_redacts(
    demo_batch, request_demo, tmp_path, monkeypatch
):
    monkeypatch.setenv("LLM_API_KEY", DUMMY_LLM_KEY)
    monkeypatch.setenv("GEMINI_API_KEY", DUMMY_GEMINI_KEY)
    monkeypatch.setenv("LLM_PROVIDER", "openai")
    ...
    with patch.object(
        ...
        side_effect=lambda req, max_retries: failover.generate_with_failover(
            ...
            generate_fn=fake_gen,
            resolve_provider=lambda: "openai",
            switch_provider=lambda name: None,
        ),
    ):
