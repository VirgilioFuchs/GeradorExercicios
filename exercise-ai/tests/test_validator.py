...
def test_non_exercise_batch_raises_without_prefix_and_logs_stderr(make_request):
    ...
    with contextlib.redirect_stderr(buf):
        with pytest.raises(ValueError) as exc_info:
            validate_exercise_batch({"exercicios": []}, req)  # type: ignore[arg-type]
...
def test_chave_exercicios_ausente_via_missing_attr(make_request, make_exercise):
    ...
    batch = _MissingExercicios.model_construct(
        exercicios=[make_exercise()],
    )
    buf = io.StringIO()
    with contextlib.redirect_stderr(buf):
        with pytest.raises(ValueError) as exc_info:
...
def test_wrong_quantity_esperado_recebido(make_request, make_exercise):
    req = make_request(quantidade=2)
    bad_qty = ExerciseBatch(exercicios=[make_exercise()])
    buf = io.StringIO()
    with contextlib.redirect_stderr(buf):
        with pytest.raises(ValueError) as exc_info:
...
def test_whitespace_field_path_message(make_request, make_exercise):
    req = make_request(quantidade=2)
    bad_ws = ExerciseBatch(
        exercicios=[
            make_exercise(enunciado="ok", resposta="  ", explicacao="ok"),
            make_exercise(enunciado="ok", resposta="ok", explicacao="ok"),
        ]
    )
    buf = io.StringIO()
    with contextlib.redirect_stderr(buf):
...
def test_report_mode_all_vs_first_exercise(make_request, make_exercise):
    ...
    multi = ExerciseBatch(
        exercicios=[
            make_exercise(enunciado=" ", resposta="b", explicacao="c"),
            make_exercise(enunciado=" ", resposta="e", explicacao="f"),
        ]
    )
