...
from token_usage import INDISPONIVEL, begin_run, estimate_usd_from_rates, extract_gemini_usage, extract_grok_usage, extract_openai_usage, flush_token_usage, get_collector
...
def test_extract_openai_usage_and_missing_indisponivel():
    fields = extract_openai_usage(_fake_openai_completion(), model='gpt-4o-mini')
...
def test_extract_gemini_usage_metadata():
    response = SimpleNamespace(usage_metadata=SimpleNamespace(prompt_token_count=50, candidates_token_count=10, total_token_count=60))
...
def test_extract_grok_prefers_cost_in_usd_ticks():
    ticks = 25000000
    fields = extract_grok_usage(_fake_openai_completion(ticks=ticks), model='grok-4.6')
    assert fields.usd == pytest.approx(ticks / 10000000000.0)
    assert fields.usd_source == 'api'
    no_ticks = extract_grok_usage(_fake_openai_completion(), model='grok-4.6')
...
def test_record_flush_ndjson_append_and_idempotent(usage_dir, capsys):
    c = get_collector()
    fields = extract_openai_usage(_fake_openai_completion(), model='gpt-4o-mini')
    c.record(provider='openai', model='gpt-4o-mini', status='success', prompt_tokens=fields.prompt_tokens, completion_tokens=fields.completion_tokens, total_tokens=fields.total_tokens, duration_ms=42, usd=fields.usd, usd_source=fields.usd_source)
    ...
    begin_run('testrun02')
    c.record(provider='openai', model='gpt-4o-mini', status='success', prompt_tokens=5, completion_tokens=1, total_tokens=6, duration_ms=1, usd=INDISPONIVEL, usd_source='indisponivel')
    flush_token_usage()
...
def test_missing_usage_never_fabricates_zero(usage_dir, capsys):
    c = get_collector()
    c.record(provider='openai', model='unknown-x', status='error', duration_ms=3, error_kind='Timeout')
    err = capsys.readouterr().err
    assert INDISPONIVEL in err
    assert 'in=0' not in err
    flush_token_usage()
    day = datetime.now().astimezone().date().isoformat()
    row = json.loads((usage_dir / day / 'openai.ndjson').read_text(encoding='utf-8').strip())
...
def test_usage_stderr_redacts_secrets_spirit(usage_dir, monkeypatch, capsys):
    ...
    c = get_collector()
    c.record(provider='openai', model='gpt-4o-mini', status='success', prompt_tokens=10, completion_tokens=2, total_tokens=12, duration_ms=5, usd=0.001, usd_source='rate_table')
    err = capsys.readouterr().err
    assert 'sk-secret' not in err
    assert 'AIza-secret' not in err
    assert 'Resolva a equação' not in err
    flush_token_usage()
    day = datetime.now().astimezone().date().isoformat()
...
def test_rely_attempt_status_retag(usage_dir, monkeypatch, capsys):
    ...
    import reliability
    request = GenerationRequest(materia='Matemática', topico='soma', dificuldade=DificuldadeEnum.FACIL, quantidade=1)
    good = ExerciseBatch(exercicios=[Exercise(enunciado='2+2', resposta='4', explicacao='dois mais dois', dificuldade=DificuldadeEnum.FACIL)], dificuldades=[DificuldadeEnum.FACIL])
    bad = ExerciseBatch(exercicios=[Exercise(enunciado='bad', resposta='x', explicacao='y', dificuldade=DificuldadeEnum.FACIL)], dificuldades=[DificuldadeEnum.FACIL])
    calls = {'n': 0}

    def fake_gen(_req):
        calls['n'] += 1
        batch = bad if calls['n'] == 1 else good
        get_collector().record(provider='openai', model='gpt-4o-mini', status='success', prompt_tokens=10, completion_tokens=2, total_tokens=12, duration_ms=1, usd=0.01, usd_source='rate_table')
        return batch
...
def test_end_of_run_summary_numeric_vs_indisponivel(usage_dir, capsys):
    c = get_collector()
    c.record(provider='openai', model='gpt-4o-mini', status='success', prompt_tokens=100, completion_tokens=20, total_tokens=120, duration_ms=10, usd=0.05, usd_source='rate_table')
    c.record(provider='gemini', model='unknown', status='success', prompt_tokens=50, completion_tokens=10, total_tokens=60, duration_ms=8, usd=INDISPONIVEL, usd_source='indisponivel')
    flush_token_usage()
    err = capsys.readouterr().err
...
def test_main_run_flush_in_finally(usage_dir, monkeypatch, tmp_path, capsys):
    ...
    def fake_validated(req, max_retries):
        get_collector().record(provider='openai', model='gpt-4o-mini', status='success', prompt_tokens=1, completion_tokens=1, total_tokens=2, duration_ms=1, usd=INDISPONIVEL, usd_source='indisponivel')
        return batch
