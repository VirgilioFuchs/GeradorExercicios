...
def test_failover_print_to_cp1252_stderr_does_not_raise(monkeypatch):
    ...
    batch = ExerciseBatch(exercicios=[Exercise(enunciado='e', resposta='1', explicacao='x', dificuldade=DificuldadeEnum.FACIL)], dificuldades=[DificuldadeEnum.FACIL])
    calls = {'n': 0}
    ...
    def _gen(_request, _max_retries):
        ...
        if calls['n'] == 1:
            ...
            raise GenerationFailedError('Tempo esgotado', retriable=False, api_error_kind='timeout')
        return batch
    monkeypatch.setenv('LLM_PROVIDER', 'openai')
    monkeypatch.setenv('GEMINI_API_KEY', 'dummy-gemini-key')
    result = failover.generate_with_failover(req, max_retries=0, generate_fn=_gen, switch_provider=lambda _p: None)
...
def test_validation_failure_log_safe_on_cp1252(monkeypatch):
    ...
    stream = _cp1252_stream()
    monkeypatch.setattr(sys, 'stderr', stream)
    batch = ExerciseBatch(exercicios=[Exercise(enunciado='Calcule √16 + π', resposta='4 + π', explicacao='√16 = 4; use π', dificuldade=DificuldadeEnum.FACIL)], dificuldades=[DificuldadeEnum.FACIL])
...
def test_format_batch_text_printable_via_safe_path():
    ...
    batch = ExerciseBatch(exercicios=[Exercise(enunciado='√9 = ?', resposta='3', explicacao='√9 = 3', dificuldade=DificuldadeEnum.FACIL)], dificuldades=[DificuldadeEnum.FACIL])
...
def test_reconfigure_stdio_when_supported():
    ...
    try:
        ...
        assert sys.stderr.errors == 'replace' or sys.stderr.encoding.lower().startswith('utf')
    finally:
